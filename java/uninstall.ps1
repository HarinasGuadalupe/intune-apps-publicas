...
#
# El instalador de Oracle JRE 8 usa Windows Installer (MSI) por debajo.
# El ProductCode (GUID) cambia en cada actualizacion, asi que se busca
# dinamicamente en el registro por DisplayName en vez de tener el GUID
# fijo en este script.
# =============================================
...
try {
    Write-Log "Iniciando desinstalacion de Java 8 Update 503 (Oracle JRE x64)"
    ...
    if (-not $entry) {
        Write-Log "No se encontro una instalacion de Java 8 Update 503 para desinstalar."
        exit 0
    }
