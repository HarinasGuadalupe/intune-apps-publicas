# =============================================
# Install script
# App: Google Chrome (Enterprise MSI, x64)
# =============================================

$ErrorActionPreference = "Stop"
$logDir = "C:\ProgramData\IntuneLogs"
$logFile = Join-Path $logDir "chrome.log"
...
try {
    Write-Log "Iniciando instalacion de Google Chrome (Enterprise MSI x64)"

    # 1. Forzar TLS 1.2+ (evita fallos de descarga en equipos con PowerShell viejo)
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12

    # 2. URL de descarga (permalink oficial "siempre ultima version" de Google - MSI Enterprise 64-bit)
    ...
    if ($signature.Status -ne "Valid" -or $signature.SignerCertificate.Subject -notmatch "O=Google LLC") {
        Write-Log "ERROR: firma digital invalida o no corresponde a Google LLC. Abortando instalacion."
